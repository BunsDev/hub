...
export const validateUploadedWrapper = (files: any[]): [boolean, WrapperReqFiles] => {
...
export const uploadToIPFS = async (files: File[]) => {
  ...
  const filesFormatted = files.map((file) => ({ path: file.name, content: file }))

  //@ts-ignore
  const uploadedFiles: { name: string; hash: string }[] = await ipfsPlugin._ipfs.add(
    filesFormatted,
    {
